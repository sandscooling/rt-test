...
Read `_agent-docs/project-context.md` before writing code: it holds this project's technical directions, including the workspace layout, tooling, and test mechanics. Reviewers check diffs against `_agent-docs/code-review-checklist/`. `_agent-docs/rule-maintenance-guide.md` governs both, and this file keeps only working conventions and product guarantees.
...
- Never let the context compact. When the user asks, or the context passes about 60% of its window (75% for the orchestrator), hand off to a successor session by following `_agent-docs/handoff.md` (the `/handoff` skill in Claude Code).
