...
Use Node from the supported release lines in `package.json` and Bun 1.3.14. Install with `bun install --frozen-lockfile`. Work on `main` and run `bun run check` before submitting code changes.
